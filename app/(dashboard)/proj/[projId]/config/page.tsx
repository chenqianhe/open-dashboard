...
export default async function ConfigPage({ params }: { params: { projId: string } }) {
  const config = await getConfig(params.projId);
    ...
    <div className="container flex items-start justify-center min-h-[calc(100vh-4rem)] pt-36">
      <div className="w-full max-w-3xl space-y-6">
        ...
        <div className="border rounded-lg p-6">
          <ConfigForm projId={params.projId} initialData={config || undefined} />
        </div>
      </div>
    </div>
  );
