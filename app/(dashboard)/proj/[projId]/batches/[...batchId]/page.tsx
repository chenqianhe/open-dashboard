...
export default async function BatchPage({ params }: BatchPageProps) {
  ...
  const batch = await getBatchDetail(params.projId, batchId);
  ...
  if (batchData?.output_file_id) {
    const outputFileInfo = await getFileInfo(params.projId, batchData.output_file_id);
    if (outputFileInfo.success) {
      outputFileName = outputFileInfo.data.filename;
    } 
  }
  if (batchData?.error_file_id) {
    const errorFileInfo = await getFileInfo(params.projId, batchData.error_file_id);
          ...
          <div className="grid grid-cols-[160px_1fr] gap-4">
            {[
              ...
              <Fragment key={label}>
                ...
                <Link
                  href={`/proj/${params.projId}/files/${fileId}`} 
                  target="_blank"
      ...
      <div className="flex-none h-fit">
        ...
        <div className="px-6 py-2 min-h-14">
          {batchData.status !== 'cancelled' &&
            ...
            <CancelBatchButton projId={params.projId} batchId={batchId} />
          }
          {(batchData.status === 'completed' ||
            batchData.status === 'failed' ||
            batchData.status === 'cancelled') && 
            (batchData.output_file_id || batchData.error_file_id) && (
              <DownloadOutputButton projId={params.projId} fileId={batchData.output_file_id || batchData.error_file_id!}>
                Download Output
