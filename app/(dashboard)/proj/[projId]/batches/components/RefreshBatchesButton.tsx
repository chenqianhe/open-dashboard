...
export function RefreshBatchesButton({ projId }: { projId: string }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
        const batchesResult = await fetch(`/api/projects/${projId}/batches?refresh=true&limit=100&offset=0`);
        if (batchesResult.ok) {
            const batches = await batchesResult.json() as { success: boolean, data: OpenAI.Batches.Batch[], error?: string };
            if (batches.success) {
                setBatches(batches.data);
                toast({
                    title: "Batches refreshed",
                    description: "Batches and caches have been refreshed",
                });
            } else {
                toast({
