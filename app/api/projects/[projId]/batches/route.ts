...
export const GET = async (request: Request, { params }: { params: { projId: string } }) => {
    const { searchParams } = new URL(request.url);
    const offset = searchParams.get("offset") || "0";
    const limit = searchParams.get("limit") || "20";
    const refresh = searchParams.get("refresh") === "true";
    return NextResponse.json(await listBatches(params.projId, {
...
export const POST = async (request: Request, { params }: { params: { projId: string } }) => {
    ...
    const { baseUrl, apiKey } = await getBaseUrlAndKey(kv, params.projId);
